...
class My_Calendar {
	...
	public function init($comp, $link, $year = 0, $month = 0, $day = 0, $raw = true) {
		...
		if ($link == null) {
			$link = Basic::requestURI(array('year', 'month', 'day'));
		}
		...
		if ($cm != $this->month || $cy != $this->year || !$this->mark_today) {
			$this->today = 0;
		}
	}
	...
	private function tooltip($tipList, $val, $link) {
		...
		for ($i = 0;$i < count($tipList);++$i) {
			...
			$name = str_replace(array("'", '"'), ' ', $tipList[$i]->name);
			$tips .= htmlspecialchars($name);
		}
		//$tips = addslashes( $tips );
		echo '<a href="' . $link . '" onmouseover="showPopUp(this,\'' . $tips . '\');" class="tooltip" onmouseout="hidePopUp(this);">';
		echo $val;
		echo '</a>';
	}
	...
	public function show() {
		...
		echo '<caption>';
		if ($this->month == 1) self::genLink($this->year - 1, 12);
		else self::genLink($this->year, $this->month - 1);
		echo '&laquo;' . KPREV . '</a>&nbsp;';
		echo $month_name[$this->month] . ' ' . $this->year . '&nbsp;';
		if ($this->month == 12) self::genLink($this->year + 1, 1);
		else self::genLink($this->year, $this->month + 1);
		...
		echo '<tr>';
		if ($dy > 0) for ($j = 0;$j < $dy;++$j) echo '<td>&nbsp;</td>';
		for ($i = 1;$i <= $num_days;++$i) {
			echo $this->today != $i ? '<td>' : '<td class="today">';
