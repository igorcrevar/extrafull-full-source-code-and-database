...
function getXMLHTTPObject() {
	var oAsync = false;   
	if (window.XMLHttpRequest) oAsync = new XMLHttpRequest();
	else if (window.ActiveXObject){ 
 		try {oAsync = new ActiveXObject("Msxml2.XMLHTTP");} 
 		catch (e){
  		try{oAsync = new ActiveXObject("Microsoft.XMLHTTP");}
  		catch (e){}
 		}
	}else {return false;}
	return oAsync;
...
function commentChanged(textarea,maxChars,spanId)
{
  if (textarea.value.length<=maxChars){
	    ...
	    if (cl) cl.innerHTML=(maxChars-textarea.value.length)+'';
  }
