...
class JFilterInput
{
	...
	public function _cleanTags($source)
	{
		$preTag		= null;
		$postTag	= $source;
		$currentSpace = false;
	...
	public function _decode($source)
	{
		// entity decode
		...
		// convert decimal
		$source = preg_replace_callback('/&#(\d+);/', 'convert_decimal_add1', $source); // decimal notation
		// convert hex
		$source = preg_replace_callback('/&#x([a-f0-9]+);/i', 'convert_decimal_add2', $source); // hex notation
		return $source;
	}
}

function convert_decimal_add1($v) {
echo chr(intval($v)).'  '.$v.' ';
  return chr(intval($v));
}

function convert_decimal_add2($v) {
echo chr(hexdec($v)).'  '.$v.' ';
  return chr(hexdec($v));
}