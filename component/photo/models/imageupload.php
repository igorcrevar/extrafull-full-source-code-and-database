...
class ModelPhotoImageUpload extends JModel {
    ...
    public function copyFiles($id, $ajax = true) {
        $private = $this->private;
        $MAX_FOR_USER = 20;
        $user = User::getInstance();
        if ($user->gid < 18 || ($user->gid < 21 && $private == 0)) return -10110;
        if (isset($_FILES['upload_file']) && $_FILES['upload_file']['error'] == UPLOAD_ERR_OK && $id > 0) {
            ...
            if ($imgTypes[$ext]) {
                ...
                if ($private == 1) {
                    ...
                    $db->setQuery('REPLACE INTO #__changes SET who_id=' . $user->id . ',what=3,time=' . $tm . ',tag=\'' . $insertId . '\'');
                    $db->query();
                }
                else {
    ...
    public function getEvents() {
        $user = & User::getInstance();
        $a_id = $user->id;
        if ($this->private == 0) $query = "SELECT a.id,CONCAT(DATE(a.date),' @ ',b.name) AS name from #__photo_events AS a JOIN #__photo_locations AS b ON a.l_id=b.id WHERE a.published<2 AND a.a_id=$a_id  ORDER BY a.id DESC LIMIT 8";
        else $query = "SELECT id,name from #__photo_events WHERE a_id=$a_id AND published=2 ORDER BY name";
    ...
    public function getEvent($event_id) {
        $user = & User::getInstance();
        $a_id = $user->id;
        if ($this->private == 0) $query = "SELECT * from #__photo_events WHERE published<2 AND a_id=$a_id AND id=$event_id";
        else $query = "SELECT * from #__photo_events WHERE published=2 AND a_id=$a_id AND id=$event_id";
        $rows = $this->_getList($query);
        return $rows[0];
    }

	public function getCatLoc($row) {
