...
class Securimage {
  ...
  {
  	$this->rSession = &Session::getInstance();
  	/*
  ...
  function drawWord()
  {
    if ($this->use_gd_font == true) {
      ...
      imagestring($this->im, $font, $this->text_x_start, ($this->image_height / 2) - ($this->gd_font_size / 2), $this->code, $color);
    	...
    	$this->ttf_file = BASE_PATH.DS.'component'.DS.'user'.DS.'captcha'.DS.$this->ttf_file;
  ...
  function generateCode($len)
  {
    ...
    for($i = 1, $cslen = strlen($this->charset); $i <= $len; ++$i) {
      $ind = rand(0, $cslen - 1);
      $code .= strtoupper( $this->charset[$ind] );
    }
    return $code;
  }
  ...
  function saveData()
  {
  	$this->rSession->set('securimage_code', strtolower($this->code) );
  ...
  {
  	$s = $this->rSession;
  	$val = $this->rSession->get('securimage_code', '' );
    if ( !empty($val) ) {
      if ( $val == strtolower(trim($this->code_entered)) ) {
